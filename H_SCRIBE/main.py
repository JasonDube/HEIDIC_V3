"""
H_SCRIBE - minimal pygame-based HEIDIC script editor scaffold.
...
class Editor:
    ...
    def _do_build(self, hd_path, cpp_path):
        ...
        # Get project root (two levels up from H_SCRIBE)
        script_dir = os.path.dirname(os.path.abspath(__file__))
        project_root = os.path.abspath(os.path.join(script_dir, ".."))
...
def project_picker_dialog(screen, editor):
    """Simple project picker that lists all .hd files in H_SCRIBE/PROJECTS/"""
    font = pygame.font.SysFont("Consolas", 16)
    dialog_width = 600
    dialog_height = 500
    dialog_x = (SCREEN_WIDTH - dialog_width) // 2
...
def main():
    ...
    pygame.display.set_caption("H_SCRIBE - HEIDIC Script Editor")
    clock = pygame.time.Clock()
    editor = Editor(path)
                    ...
                    for rect, icon_char, tooltip, action in editor.menu_hitboxes:
                        if rect.collidepoint(local_x, local_y):
                            if action == "new":
                                ...
                                if project_name:
                                    # Create project folder in H_SCRIBE/PROJECTS/
